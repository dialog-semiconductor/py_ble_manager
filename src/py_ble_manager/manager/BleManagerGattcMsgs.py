...
class BleMgrGattcBrowseRangeCmd(BleMgrMsgBase):
    def __init__(self,
                 conn_idx: int = 0,
                 start_h: int = 0,
                 end_h: int = 0,
                 uuid: AttUuid = None) -> None:
        super().__init__(opcode=BLE_CMD_GATTC_OPCODE.BLE_MGR_GATTC_BROWSE_RANGE_CMD)
        self.conn_idx = conn_idx
...
class BleMgrGattcBrowseRangeRsp(BleMgrMsgRsp):
    def __init__(self, status: BLE_ERROR = BLE_ERROR.BLE_ERROR_FAILED) -> None:
        super().__init__(opcode=BLE_CMD_GATTC_OPCODE.BLE_MGR_GATTC_BROWSE_RANGE_CMD,
                         status=status)
