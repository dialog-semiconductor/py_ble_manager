from ..gtl_messages.gtl_message_gapc import GapcConnectionReqInd, GapcConnectionCfm, GapcSecurityCmd, GapcCmpEvt, GapcGetInfoCmd, \
    ...
    GapcSecurityInd, GapcSignCounterInd, GapcSetDevInfoReqInd, GapcLePhyInd
    ...
    def create_message(msg_bytes: bytes):
        ...
        try:
            if msg_id == GAPC_MSG_ID.GAPC_CONNECTION_REQ_IND:
                ...
                req = params_buf[0]
                parameters.req = req
                if req == GAPC_DEV_INFO.GAPC_DEV_NAME:
                    parameters.info.name.value = params_buf[2:]  # index from 2 due to padding in gapc_set_dev_info_req_ind
                else:
